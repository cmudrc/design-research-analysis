...
PUBLIC_API_INIT = REPO_ROOT / "src" / "design_research_analysis" / "__init__.py"
...
def _discover_examples() -> tuple[Path, ...]:
    ...
    for pattern in ("*.py", "*.ipynb"):
        for path in sorted(EXAMPLES_ROOT.rglob(pattern)):
            ...
            if "__pycache__" in parts or path.name.startswith("_"):
                continue
...
def _collect_package_aliases(module: ast.Module) -> set[str]:
    ...
    Returns:
        Alias names bound to direct ``import design_research_analysis`` statements.
    """
    aliases: set[str] = set()
