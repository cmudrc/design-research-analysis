...
- Module facades: ``dataset``, ``embedding_maps``, ``language``, ``runtime``, ``sequence``, ``stats``
...
- Embedding maps: ``EmbeddingResult``, ``EmbeddingMapResult``, ``embed_records``,
  ``build_embedding_map``, ``cluster_embedding_map``, ``compare_embedding_maps``,
